...
function jqNavSelect(tDoc, nav){
  ...
  var heading = [];
//  var nav = DOMList("ul", "Navigation", "navselect", false);
...
//  hash = hash.length ? hash.split(".").map(Number) : false;
  ...
  for(var i=0; i<structures.length; i++){
    ...
    if(sbreak.length){
      ...
      // $(option).data("ref", (reference==="Prol." ? "prologue"
      //                        : (reference==="Conc." ? "conclusion"
      //                           : (section ? [book, chapter, section] : [book, chapter]).join("."))));
...
//        window.location.hash = "#"+$(this).data("ref");
        ...
        // pageSettings.updateSettings([["book", s[0]], ["chapter", s[1]], ["section", s[2]]]);
        // var treatise = $(this).data("treatise");
        // var docs = docMap.treatises[treatise];
        // for(var i=0; i<docs.length; i++){
        //   simpleScrollTo(docs[i].drawTo, 0, s[0], s[1], s.length>2 ? s[2] : 0, 0);
        // } 
      });
    }
  }
  // var navMenu = DOMListItem(false, false, "Go to…");
  // navMenu.appendChild(DOMList("ul", false, false, options));
